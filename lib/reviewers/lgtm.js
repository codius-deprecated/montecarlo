  ...
  checkAndMergePR: function(buildSucceeded, metadata, pr) {
    ...
    if (pr.state.indexOf("open") == -1) {
      console.log("%s/%s/%s is not open, it is %s",
          ...
          pr.number,
          pr.state);
      return self.reviewer.redis.hsetAsync('pr:'+pr.id, 'reviewState', 'finished');
    }
      ...
      if (metadata.lgtm.length >= self.threshold) {
        if (buildSucceeded) {
          ...
          console.log("Build for %s/%s/%s did not succeed, but we have enough LGTMs. Should probably poke someone, eh?",
              ...
              self.reviewer.repo,
              pr.number);
          return self.reviewer.redis.hsetAsync('pr:'+pr.id, 'reviewState', 'build-failure');
        ...
        return self.reviewer.github.issues.createCommentAsync({
          ...
          return self.reviewer.redis.hsetAsync('pr:'+pr.id, blip);
        });
      } else {
        console.log("%s/%s/%s needs %d more LGTMs.", self.reviewer.user, self.reviewer.repo, pr.number, self.threshold - metadata.lgtm.length);
        return self.reviewer.redis.hsetAsync('pr:'+pr.id, 'reviewState', 'needs-lgtms');
        // TODO: Notify in slack or via github pings
      }
    } else {
      console.log('No review requested yet.');
      return self.reviewer.redis.hsetAsync('pr:'+pr.id, 'reviewState', 'pre-review');
    }
  },
