...
TrackerProcessor.prototype = {
    ...
    stories.forEach(function(s) {
      if (pr.state == "open") {
        p.push(self.getReviewLabel().then(function(label) {
          return self.markStoryDelivered(label, s, pr)
        }));
      } else if (pr.state == "closed") {
        p.push(self.reviwer.github.pullRequests.getMergedAsync({
          repo: self.reviewer.repo,
          user: self.reviewer.user,
          number: pr.number
        }).then(function() {
          return self.markStoryAccepted(s, pr)
        }));
      }
    });
  ...
  markStoryDelivered: function(label, id, pr) {
    var self = this;
  ...
  markStoryAccepted: function(label, id, pr) {
    var self = this;
    var storyObj = self.project.story(id);
    return storyObj.getAsync().then(function(story) {
